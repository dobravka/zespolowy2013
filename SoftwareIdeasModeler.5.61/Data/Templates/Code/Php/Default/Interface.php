...
<%==SIM:EndFor==%><%==SIM:If:Element.IsAbstract==%> abstract<%==SIM:EndIf==%> interface <%==SIM:Element.Name==%><%==SIM:If:Element.HasInterfaces==%> extends <%==SIM:ForEach:Element.Interfaces==%><%==SIM:Interface.Name==%><%==SIM:IfNot:IsLastItem==%>, <%==SIM:EndIf==%><%==SIM:EndFor==%><%==SIM:EndIf==%>
{
